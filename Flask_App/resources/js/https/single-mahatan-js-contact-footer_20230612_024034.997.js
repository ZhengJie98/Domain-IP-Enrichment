...
$(document).ready(function () {
  ...
  $("#btn_contact").on("click", function () {
    if ($("#contactForm").valid()) {
      if (enable_captCha == 1 && useGoogleCaptcha == 0) {
      ...
      if ($("select.unittypes").length > 0) {
        unit_types = $("select.unittypes").val().toString();
      }
      ...
      if (enable_appointment_date == 1) {
        ...
        data["description"] = $(".show-appointment-time option:selected").text() + " - " + description.val();
      }
...
if ($("#contactForm").length > 0) {
  var contactformWidth = $(".project-tabs-roll-box").width(),
    f = $("#contactForm"),
    fot = f.offset().top,
    fol = f.offset().left,
    hh = $("header").height(),
    fh = $("#news-footer").height() + $("footer.newfooter").height();
  if ($("#news-footer").height() < 700) {
    fh = fh - 30;
  } else {
    fh = fh - 120;
  }
  ...
  $(document).scroll(function () {
    if ($(document).scrollTop() > fot - (hh + 35)) {
      var top = hh + 35 + "px",
        bottom = "auto";
      if ($(document).scrollTop() + $(window).height() >= $(document).height() - fh) {
        top = "auto";
        bottom = fh + 50 + "px";
      }
      f.parents(".project-tabs-roll-box").attr("style", "position: fixed; top: " + top + "; bottom: " + bottom + "; left:" + fol + "px;");
    } else {
      f.parents(".project-tabs-roll-box").removeAttr("style");
    }
  });
}
...
function initRulesContactForm2() {
  $("#name2").rules("add", {
    required: true,
    minlength: 2,
  });
...
function checkPhone(el) {
  ...
  //sin
  var regex65 = /(\+65|65|(65)|(\+65)|\+65 |65 |\(65\) |\(\+65\) |)(6|8|9)(\d{7}|\d{3} \d{4})/g;
  ...
  var countryName = $(el).intlTelInput("getSelectedCountryData").name;
  $('<label id="form-contact-phone-error" class="error" for="form-contact-phone">Please enter a valid ' + countryName + " phone.</label>").insertAfter($(el));
  return false;
}
