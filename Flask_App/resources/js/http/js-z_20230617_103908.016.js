$(function(){
	...
	$('.i_video li:last-child').css('margin-right',0+'px');
	$('.i_member dl:nth-child(3n)').css('margin-right',0+'px');
	$('.member dl:nth-child(2n)').css('padding-left',35+'px');
	
	$('.scd_l ul li .li_m').click(function(){
		$(this).parents('li').siblings('li').removeClass('on');
		$(this).parents('li').addClass('on');
	});
	
	$('.pro .pro_a ul li').mousemove(function(){
		$(this).siblings('li').removeClass('on');
		$(this).addClass('on');
		var url =$(this).children("img").attr("src");
		$(".pro .pro_a .tu").find("img").attr("src",url);
	});
