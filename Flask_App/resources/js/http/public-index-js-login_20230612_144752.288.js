...
(function () {
    ...
    var regex4email = /^([a-zA-Z]|[0-9])(\w|\-)+@[a-zA-Z0-9]+\.([a-zA-Z]{2,4})$/;
    ...
    function initBirth() {
        ...
        for (i = (currentYear - 18); i > (currentYear - 106); i--) {
            option = new Option();
            option.innerHTML = option.value = i;
    ...
    function reg() {
        ...
        var reg_pwd = cm.get("reg_pwd").val();
      	var reg_pwd2 = cm.get("reg_pwd2").val();
		...
		//console.log(reg_year,reg_month,reg_day)
        var checkCount = 0;
        ...
        //f("reg_birth", reg_year != -1 && reg_month != -1 && reg_day != -1, document.i18n["tip_birth"]);
		 if(!reg_email || !reg_email.match(regex4email)){
            layer.msg(document.i18n["tip_email"])
        }else if(!reg_alias){
            layer.msg(document.i18n["tip_alias"])
        }else if(!regex4pwd.test(reg_pwd)){
            layer.msg(document.i18n["tip_pwd"])
        }else if(!reg_pwd2){
            layer.msg(document.i18n["tip_pwd"])
        }else if(reg_year == -1 && reg_month == -1 && reg_day == -1){
            layer.msg(document.i18n["tip_birth"])
        }else{
            ...
            cm.ajax("index.php?m=Index&c=Index&a=register", {username:reg_alias,pwd:reg_pwd,email: reg_email,sex:reg_gender,year:reg_year,month:reg_month,day:reg_day}, function (res) {
             // console.log(777,res)
                if (res["status"] == 'success') {
                	layer.msg(document.i18n["being success"], {
                    ...
                    layer.close(index);
                    layer.msg(document.i18n["tip_username_exist"]);
                    f("reg_alias", false, document.i18n["tip_username_exist"]);
                } else if (res["status"] == 2){
                    layer.close(index);
                    layer.msg(document.i18n["tip_email_exist"]);
                    f("reg_email", false, document.i18n["tip_email_exist"]);
                } else if (res["status"] == 3){
                    layer.close(index);
                    layer.msg(document.i18n["reg_failed"]);
                }else if (res["status"] == 4){
                  	//alert(document.i18n["being audited"]);
                  layer.close(index);
                  layer.msg(document.i18n["being audited"], {
                    time: 3000 //2秒关闭（如果不配置，默认是3秒）
                  }, function(){
                    //do something
                     window.location = "index.php?m=Index&c=Index&a=logout"
                  }); 
                    
                }else if(res["status"] == 5){
                    layer.close(index);
                    f("reg_alias", false, document.i18n["tip_alias"]);
                }
            });
        }
    }
    ...
    function login() {
        ...
        if(!name || !pwd){
            layer.msg(document.i18n["tip_login"]);
            return;
        } else{
            var index = layer.load();
            pwd = $.md5(pwd);
