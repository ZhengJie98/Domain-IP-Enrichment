...
function pe_jstime(obj, nowtime, type, lang) {
	...
	$(obj).each(function(index){
		...
		setInterval(function(){
			...
			if (typeof(type) != 'undefined' && type == 'html') {
			    if (all_s >= 0) {
					_this.html(js_h+hour+js_m+min+js_s+sec);
			    }
			    else{
					_this.html('0'+hour+'0'+min+'0'+sec);
				}
			}
			else {
			    if (all_s >= 0) {
...
//单选/多选美化版
function pe_select_radio_lang(name, value, func) {
	if (typeof(value) != 'undefined') {
		$(":input[name='"+name+"']").live("change", function(){
			$(":input[name='"+name+"']").each(function(){
				if ($(this).is(":checked")) {
