...
var common = function () {
    ...
    this.showChat = function (u) {
        ...
        if(1){
            ...
            layer.alert(self.i18n("need_upgrade"), function(index){
                //do something
            	self.showPay({type: "u"});
                layer.close(index);
            }); 
    ...
    //gaoyuanadd
    this.showVideo = function(u){
        //console.log(this.getRole());
        level = this.getRole();
        if(!self.isMember()){
            layer.alert(self.i18n("need_recharge"),function(index){
            	self.showPay({type: "u"});
                layer.close(index);
            });
            
        }else if(self.isMember() && level == 1){
            //alert(self.i18n("need_upgrade"));
          	layer.alert(self.i18n("need_upgrade"), function(index){
                //do something
				self.showPay({type: "u"});
            ...
            //self.showPay({type: "u"});
        }else if (self.isMember() && level == 2) {
            var iframe = document.createElement('iframe');
            iframe.src="index.php?m=Index&c=index&a=video";
            iframe.id="ifr-video";
            ...
            //var dlg = self.getDlg();
            //dlg.setArgs(u);
            //dlg.showPage("index.php?m=Index&c=index&a=video", 500, 120);
        }
    }
    ...
    this.showPay = function (p) {
        ...
        dlg.hideTitle();
        dlg.setArgs(p);
        dlg.showPage("index.php?m=Index&c=Pay&a=index", 800, 600);
    };
    ...
    this.getCodeList = function () {
        return ["zh-cn", "zh-tw", "en-us", "ja", "ko-kr", "de", "es", "fra", "it", "nl", "pt", "ru", "th"];
    };
    this.getEasyCodeList = function () {
        return ["中", "繁", "EN", "日", "KO", "DE", "ES", "FR", "IT", "NL", "PT", "RU", "TH"];
    };
    this.getRawLangList = function () {
        return ["中文-简体", "中文-繁体", "English", "日本語", "한국의", "Deutsch", "Español", "Français", "Italiano", "Nederlandse", "Português", "Русский", "ไทย"];
    };
