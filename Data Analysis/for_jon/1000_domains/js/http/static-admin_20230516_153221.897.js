...
$(function () {
    ...
    $.form = new function (that) {
            ...
            $.ajax({
                data: data || {}, type: method || 'GET', url: $.menu.parseUri(url), beforeSend: function (xhr) {
                    ...
                    if (XMLHttpRequest.responseText.indexOf('exception') > -1) layer.open({
                        ...
                        area: '1000px', content: 'javascript:void(0)', success: function ($element, index) {
                            try {
                                layer.full(index);
        ...
        this.iframe = function (url, title, area) {
            return layer.open({title: title || '窗口', type: 2, area: area || ['1000px', '580px'], fix: true, maxmin: false, content: url});
        };
        // 加载HTML到弹出层
        this.modal = function (url, data, title, callback, loading, tips) {
            this.load(url, data, 'GET', function (res, index) {
                ...
                index = layer.open({
                    type: 1, btn: false, area: "1000px", content: res, title: title || '', success: function (dom, index) {
