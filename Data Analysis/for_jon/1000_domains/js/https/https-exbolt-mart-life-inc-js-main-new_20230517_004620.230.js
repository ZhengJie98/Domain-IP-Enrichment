...
function centerboxFunction(){
	...
	$(".js-centerbox,.confirm_message,.confirm_message_cancel:visible").each(function(){
		if(isMob)
		{
			...
			centerLeftt =(winwidth-$(this).outerWidth())/5;	
		}
		centerLeft =(winwidth-$(this).outerWidth())/2.6;	
		centerTop =(winheight-$(this).outerHeight())/5;
		if(centerTop<100){ //changes
			centerTop = 50;
		}
		$(this).css({
			left:centerLeftt,
...
$(function(){
 $("input[type=checkbox]").change(function(){
    ...
    function formStyleFunction(){
	    $("input[type=checkbox]").each(function(){
	    	thisID = $(this).attr("id");
	    	thisChecked = $(this).prop("checked");
	    	if(thisChecked){
	    		$(".form-label-checkbox[for="+thisID+"]").addClass("checked")
	    	}else{
	    		$(".form-label-checkbox[for="+thisID+"]").removeClass("checked")
	    	}
	    })
    }
});
